...
import { setHistory, roundIncrement, scoreIncrement, scoreDecrement } from '../redux/actions/rpsActions';
...
//Cek apakah sudah login atau belum
    ...
    rng() {
        ...
        if (this.state.numberResult == 3) {
            return this.setState({ comChoice: `${this.state.playerChoice == 'rock' ? 'paper' : (this.state.playerChoice == 'paper' ? 'scissor' : 'rock')}` })
        }
    ...
    suitCheck() {
        ...
        if (this.state.comChoice == "scissor" && this.state.playerChoice == "paper" || this.state.comChoice == "paper" && this.state.playerChoice == "rock" || this.state.comChoice == "rock" && this.state.playerChoice == "scissor") {
            ...
            return this.setState({ comScore: this.state.comScore + 1 });
        }
        this.setState({ result: result[1] });
        return this.setState({ playerScore: this.state.playerScore + 1 });
    }
...
//Pass state to database, only if win or lose
    async toDatabase (){
        ...
        axios.post("https://api-kel3.herokuapp.com/score/rps",
            {
                        ...
                        <div className={style.suit}>
                            <div id={style.player}>
                                ...
                                <div className={`${this.state.isChosen ? null : style.hover} ${this.state.playerChoice == 'rock' ? style.chosen : null}`}>
                                    <img src="./batu.png" alt="" id={style.playerRock}
                                        className={style.gambar} value='rock' onClick={this.state.isChosen ? null : this.handleOnClick}
                                    />
                                </div>
                                <div className={`${this.state.isChosen ? null : style.hover} ${this.state.playerChoice == 'paper' ? style.chosen : null}`}>
                                    <img src="./kertas.png" alt="" id={style.playerPaper}
                                        className={style.gambar} value='paper' onClick={this.state.isChosen ? null : this.handleOnClick}
                                    />
                                </div>
                                <div className={`${this.state.isChosen ? null : style.hover} ${this.state.playerChoice == 'scissor' ? style.chosen : null}`}>
                                    <img src="./gunting.png" alt="" id={style.playerScissor}
                                        className={style.gambar} value='scissor' onClick={this.state.isChosen ? null : this.handleOnClick}
                                    />
                                </div>
                            </div>
                            <div id={style.mid}>
                                <div className={style.midbar}>
                                    <div id={style.score}>
                                        <h1>{this.state.playerScore} - {this.state.comScore}</h1>
