...
class GameDetail extends Component {
  render() {
    ...
    const { isGamePlayed, isLoggedIn, setPlayed } = this.props;
    return (
      <div className="container p-4">
        ...
        <div className="border border-dark p-3 rounded">
          <p>
            Batu-Gunting-Kertas adalah sebuah permainan tangan dua orang. Permainan ini sering digunakan untuk pemilihan acak, seperti halnya pelemparan koin, dadu, dan lain-lain. Terdapat tiga isyarat tangan dalam permainan ini. Batu digambarkan oleh tangan mengepal,
        ...
        {isGamePlayed ? (<p className="lead h4 text-primary text-center fw-bolder">Game ini sudah pernah dimainkan</p>) : ''}
        <div className="d-flex justify-content-center">
          <button
            type="button"
            className="btn btn-primary btn-lg px-5"
            // eslint-disable-next-line no-unused-expressions
            onClick={() => { isLoggedIn ? (setPlayed(true), Router.push('/rps')) : (Router.push('/login')); }}
          >
            <h3>Play</h3>
          </button>
        </div>
